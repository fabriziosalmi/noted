...
describe('MCP Tool Handlers', () => {
  ...
  describe('agent state-machine tools', () => {
    ...
    // Build an agent-note HTML (marked-escaped metadata block) directly.
    const agentNoteHtml = (m: Record<string, unknown>) => {
      const json = JSON.stringify(m, null, 2)
        .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
      return `<h1>node</h1><h2>Agent Metadata</h2><pre><code class="language-json">${json}</code></pre>`;
    };

    it('locates the workflow by id even when its file was renamed, and mirrors to it', async () => {
      // Workflow note under a non-conventional name (as after a title-driven rename).
      mockFiles.set('/mockdir/noted/WF001 Renamed.md', {
        content: agentNoteHtml({
          notedAgent: true, schemaVersion: 1, type: 'workflow', id: 'WF001', approvalMode: 'autonomous',
          status: 'running', tasks: [{ id: 'T001', title: 'a', parentId: null, dependsOn: [], status: 'running' }],
        }),
        mtime: new Date(), size: 100,
      });
      mockFiles.set('/mockdir/noted/task-T001-x.md', {
        content: agentNoteHtml({
          notedAgent: true, schemaVersion: 1, type: 'task', id: 'T001', workflowId: 'WF001', dependsOn: [], status: 'running',
        }),
        mtime: new Date(), size: 100,
      });
      // autonomous workflow (found by id) → review gate not enforced → allowed.
      const res = await handleAdvanceAgentState({ name: 'noted/task-T001-x.md', to: 'verified' });
      expect(res.content[0].text).toContain('-> verified');
      const wf = readAgentMetadata(mockFiles.get('/mockdir/noted/WF001 Renamed.md')!.content);
      expect(wf?.tasks?.find(t => t.id === 'T001')?.status).toBe('verified');
    });

    it('fails safe to manual mode when a task\'s workflow is missing (gate not skipped)', async () => {
      mockFiles.set('/mockdir/noted/task-orphan.md', {
        content: agentNoteHtml({
          notedAgent: true, schemaVersion: 1, type: 'task', id: 'T001', workflowId: 'WF999', dependsOn: [], status: 'running',
        }),
        mtime: new Date(), size: 100,
      });
      // running -> verified bypasses the review gate; with no workflow found the
      // mode falls back to manual, so it must be blocked, not silently allowed.
      await expect(
        handleAdvanceAgentState({ name: 'noted/task-orphan.md', to: 'verified' }),
      ).rejects.toThrow(/review approval/);
    });
  });
  ...
  describe('main entry point', () => {
    ...
    it('starts sse transport and handles HTTP requests', async () => {
      ...
      // Two-factor: /messages also requires the token. Without it, a leaked
      // sessionId alone is rejected (401) — never routed to a transport.
      ...
      expect(mockResMsgNoToken.writeHead).toHaveBeenCalledWith(401);

      // With the token (here via the X-MCP-Token header) it passes auth and is
      // routed by sessionId — an unknown session is 404, not 401.
      const mockReqMsgToken = { method: 'POST', url: '/messages?sessionId=nope', headers: { host: 'localhost:8080', 'x-mcp-token': 'supersecret' } } as any;
      const mockResMsgToken = { writeHead: vi.fn(), end: vi.fn(), setHeader: vi.fn() } as any;
      await httpHandler(mockReqMsgToken, mockResMsgToken);
      expect(mockResMsgToken.writeHead).not.toHaveBeenCalledWith(401);
      expect(mockResMsgToken.writeHead).toHaveBeenCalledWith(404);
