...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  setActiveVaultDir: (dir: string | null) => ipcRenderer.send('set-active-vault-dir', dir),
  setLlmHosts: (hosts: string[]) => ipcRenderer.send('set-llm-hosts', hosts),
  onNoteChangedExternally: (cb: (fileName: string) => void) => {
    const listener = (_e: unknown, fileName: string) => cb(fileName);
    ipcRenderer.on('note-changed-externally', listener);
    return () => ipcRenderer.removeListener('note-changed-externally', listener);
  },
  ...
  getMcpSseToken: () => ipcRenderer.invoke('get-mcp-sse-token') as Promise<string>,
