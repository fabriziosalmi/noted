    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[12vh]">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100/40 dark:border-gray-800/40">
          ...
          />
          {query && (
            <button onClick={() => setQuery('')} aria-label={t('clearSearch')} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
              <X size={13} />
            </button>
          )}
        ...
        <div ref={listRef} className="max-h-[420px] overflow-y-auto py-1">
          {!query.trim() && (
            <div className="flex flex-col items-center gap-1.5 py-10 text-gray-400">
              ...
              <p className="text-xs">{t('globalSearchHint')}</p>
            </div>
          )}
        ...
        <div className="px-4 py-2 border-t border-gray-100/40 dark:border-gray-800/40 flex items-center gap-4 text-[11px] text-gray-400">
          ...
          <span>{t('escHint')}</span>
          {results.length > 0 && (
            <span className="ml-auto">{t(results.length === 1 ? 'searchResultsCount_one' : 'searchResultsCount_other').replace('{count}', String(results.length))}</span>
          )}
        </div>
      </div>
    </div>
