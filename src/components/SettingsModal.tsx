  ...
  const handleSetupClaudeMcp = async () => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        setSetupError(res?.error ?? t('errUnknown'));
      }
    } catch (err) {
      setSetupStatus('error');
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-gray-200/40 dark:border-gray-700/40 bg-gray-50/40 dark:bg-gray-800/25 p-3 space-y-3">
        ...
        {settings.mcpSseEnabled && (
          <div className="pt-1 space-y-3 border-t border-gray-200/40 dark:border-gray-700/40">
            ...
            {sseToken && (
              <div className="space-y-1">
                <p className="text-[11px] font-medium text-gray-600 dark:text-gray-300">{t('mcpSseAuthHeader')}</p>
                <p className="text-[10px] text-gray-400 dark:text-gray-500 leading-tight">{t('mcpSseAuthHeaderHelp')}</p>
                <CopyBlock
                  value={`X-MCP-Token: ${sseToken}`}
                  kind="mcp-sse-token"
                  copiedCmd={copiedCmd}
                  copyText={copyText}
                  label={t('copy')}
      ...
      <div className="rounded-lg border border-gray-200/40 dark:border-gray-700/40 bg-gray-50/40 dark:bg-gray-800/25 p-3 space-y-3">
        <div>
          ...
          <p className="text-[10px] text-gray-500 dark:text-gray-400 mb-1">{t('mcpCliOneLiner')}</p>
        ...
        <div className="pt-1 border-t border-gray-200/40 dark:border-gray-700/40">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {setupStatus === 'loading' ? (
                  <span>{t('mcpSetupLoading')}</span>
                ) : setupStatus === 'success' ? (
                  <span>{t('mcpSetupSuccess')}</span>
                ) : setupStatus === 'error' ? (
                  <span>{t('mcpSetupError')}</span>
                ) : (
                  <span>{t('mcpSetupCta')}</span>
                )}
              </button>
              {setupError && (
...
export function SettingsModal({ settings, onUpdate, onSelectFolder, onImportVault, onClose, onToast }: SettingsModalProps) {
  ...
  const handleActivateProvider = useCallback(async (notedPath: string) => {
    ...
    if (res.success && res.data) {
      ...
      dispatchImportWorkflow({ type: 'FAILED', message: res.error ?? t('cloudActivationFailed') });
    }
  }, [onUpdate, importWorkflow.stage, t]);
