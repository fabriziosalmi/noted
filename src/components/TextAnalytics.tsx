...
export function TextAnalytics({ getText, activeNoteName }: TextAnalyticsProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-5">
      ...
      <section>
        ...
        <div className="bg-gray-50/40 dark:bg-gray-850/30 border border-gray-100/40 dark:border-gray-700/40 rounded-lg p-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-gray-700 dark:text-gray-200">{metrics.fleschLabelKey ? t(metrics.fleschLabelKey) : '—'}</span>
          </div>
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <span className={`text-xs font-medium capitalize ${toneColors[metrics.tone]}`}>{t(toneKeys[metrics.tone])}</span>
          </div>
          <div className="flex items-center justify-between mt-1">
            <span className="text-xs text-gray-500 dark:text-gray-400">{t('analyticsAvgWords')}</span>
      ...
      {/* Metrics bar */}
      <section>
        <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">{t('analyticsDistribution')}</p>
