...
export function TextAnalytics({ getText, activeNoteName }: TextAnalyticsProps) {
  ...
  if (!metrics || !activeNoteName) {
    return (
      <div className="flex-1 flex items-center justify-center p-6 text-center">
        <p className="text-xs text-gray-400 dark:text-gray-500">{t('analyticsNoContent')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-5">
      {/* Counts */}
      <section>
        <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">{t('analyticsWords')}</p>
      ...
      {/* Readability */}
      <section>
        <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1">{t('analyticsReadability')}</p>
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
          ...
          <div className="flex items-center justify-between mt-2">
            <span className="text-xs text-gray-400 dark:text-gray-500">{t('analyticsToneLabel')}</span>
            <span className={`text-xs font-medium capitalize ${toneColors[metrics.tone]}`}>{metrics.tone}</span>
          </div>
          <div className="flex items-center justify-between mt-1">
            <span className="text-xs text-gray-400 dark:text-gray-500">Avg words/sentence</span>
      ...
      {metrics.topKeywords.length > 0 && (
        <section>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">{t('analyticsKeywords')}</p>
      ...
      {/* Metrics bar */}
      <section>
        <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2">Distribution</p>
