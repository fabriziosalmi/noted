    ...
    timerRef.current = setTimeout(async () => {
      ...
      try {
        const result = await askLLM([
          {
            role: 'system',
            content: 'You suggest tags for notes. Return ONLY a comma-separated list of 3-6 relevant tags, in lowercase, without # and without any other explanation. Use the same language as the note. Example: work, project, api, typescript',
          },
          {
            role: 'user',
            content: `Suggest tags for this note (avoid these already present: ${existingTags.join(', ')}):\n\n${text.slice(0, 1000)}`,
          },
        ]);
        const tags = result
          .split(',')
