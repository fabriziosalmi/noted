...
export function NoteEditor({ activeNoteName, activeNoteContent, saveActiveNote, onEditorReady, onWordCountChange, onAiError, allNoteNames = [], allTags = [], backlinks = [], onSelectNote, notesCount = 0, onCreateNote, onOpenDaily, onOpenSettings, onOpenShortcuts }: NoteEditorProps) {
  ...
  // The outstanding debounced autosave (target note captured at schedule time),
  ...
  const flushSave = useCallback(async (content: string) => {
    ...
    try {
      ...
      // Keep the editor buffer as-is (dirty) and surface the failure — never
      // report "Saved" over a write that didn't land.
  ...
  const debouncedSave = useCallback((content: string) => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        ...
        // Keep pendingSaveRef so a later flush (switch/close) retries the write.
        if (mountedRef.current) setSaveStatus('error');
      }
    }, 600);
  }, [saveActiveNote]);
  ...
  // Reliable flush on app quit: the main process holds the quit until we drain
  // pending saves and acknowledge (the async pagehide path can race exit).
  useEffect(() => {
    ...
    <>
      ...
      {editor && (
        ...
        <BubbleMenu editor={editor} tippyOptions={{ duration: 100 }} className="flex space-x-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg p-1">
          <button type="button" aria-label={t('bold')} onClick={() => editor.chain().focus().toggleBold().run()}
          ...
          <button type="button" aria-label={t('italic')} onClick={() => editor.chain().focus().toggleItalic().run()}
          ...
          <button type="button" aria-label={t('strikethrough')} onClick={() => editor.chain().focus().toggleStrike().run()}
      ...
      <div className="fixed bottom-4 right-4 flex items-center gap-3 z-20">
        ...
        {wordCount > 0 && (
          <span className="text-xs text-gray-500 dark:text-gray-400">{wordCount} {t(wordCount === 1 ? 'word' : 'words')}</span>
        )}
        {saveStatus !== 'idle' && (
          <div aria-hidden="true" className={`text-xs px-2.5 py-1 rounded-full flex items-center gap-1.5 ${
            saveStatus === 'saving' ? 'text-gray-400 dark:text-gray-500'
              : saveStatus === 'error' ? 'text-red-600 dark:text-red-400 bg-red-500/10 font-medium'
              : 'text-emerald-600 dark:text-emerald-400'
          }`}>
            {saveStatus === 'saving' ? <span>{t('saving')}</span>
              : saveStatus === 'error' ? <><AlertTriangle size={13} /><span>{t('saveFailed')}</span></>
              : <><CheckCheck size={13} /><span>{t('saved')}</span></>}
          </div>
        )}
        {/* SR live regions: "Saved" milestone (polite), save failure (assertive). */}
        <span className="sr-only" role="status" aria-live="polite">{saveStatus === 'saved' ? t('saved') : ''}</span>
        <span className="sr-only" role="alert">{saveStatus === 'error' ? t('saveFailed') : ''}</span>
      </div>
    </>
  );
}
