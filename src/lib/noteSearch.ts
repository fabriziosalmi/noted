...
async function fetchEmbedding(provider: HybridRetrievalConfig['provider'], model: string, input: string, apiKey?: string, lmStudioUrl?: string): Promise<number[]> {
  ...
  if (provider === 'openai') {
    ...
    // Mask PII before embedding text leaves the machine to OpenAI (unless
    // masking was explicitly turned off).
    const safeInput = useStore.getState().settings.piiMasking !== false ? maskPii(input).maskedText : input;
    const res = await transportFetch('https://api.openai.com/v1/embeddings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
      body: JSON.stringify({ model, input: safeInput }),
    });
