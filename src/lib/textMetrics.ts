...
export interface TextMetrics {
  ...
  fleschScore: number;
  /** i18n key for the readability band, or '' when there is no content. */
  fleschLabelKey: TranslationKey | '';
  avgWordsPerSentence: number;
  topKeywords: { word: string; count: number }[];
  tone: 'formal' | 'neutral' | 'informal';
}

function fleschLabelKey(score: number): TranslationKey {
  if (score >= 90) return 'fleschVeryEasy';
  if (score >= 80) return 'fleschEasy';
  if (score >= 70) return 'fleschFairlyEasy';
  if (score >= 60) return 'fleschStandard';
  if (score >= 50) return 'fleschFairlyDifficult';
  if (score >= 30) return 'fleschDifficult';
  return 'fleschVeryDifficult';
}

export function computeMetrics(plainText: string): TextMetrics {
  const text = plainText.trim();
  if (!text) {
    return { words: 0, chars: 0, sentences: 0, paragraphs: 0, readingTimeMin: 0, fleschScore: 0, fleschLabelKey: '', avgWordsPerSentence: 0, topKeywords: [], tone: 'neutral' };
  }
  ...
  return {
    ...
    fleschLabelKey: fleschLabelKey(clampedFlesch),
